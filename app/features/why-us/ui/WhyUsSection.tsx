...
export const WhyUsSection: FC = () => {
	...
	return (
		...
		>
			...
			<div className='absolute top-1/5 left-40 w-48 h-48 rounded-full bg-sky-600/90 blur-3xl animate-pulse' />
					...
					<h2 ref={thesisRef} className='text-4xl lg:text-6xl relative'>
						{whyList.thesis.map((item: string) => (
							<span
								key={item}
								className={cn(
									'text-transparent bg-clip-text bg-gradient-to-tr from-neutral-950 to-sky-600 dark:from-slate-100 dark:to-sky-600',
				...
				<div className='flex flex-col items-center justify-center gap-6 min-w-[40%]'>
					{whyList.pluses.map((item: Pluses, index: number) => (
						<div
							...
							className={cn(
								'font-mono backdrop-blur-sm border border-neutral-950/90 dark:border-sky-600/90 rounded-lg p-6 transition-all duration-200 group flex flex-col gap-3',
							...
							<div className='flex flex-wrap gap-4'>
								{item.tech.map((tech: string) => (
									<span
										...
										className={cn(
											DASHED_BACKGROUND,
											'border px-3 py-1 rounded-full text-sm text-sky-500 border-neutral-950/60 dark:border-sky-400'
										)}
									>
										{tech}
									</span>
